    ...
    public function run(): void
    {
        ...
        for ($i = 0; $i < 3; $i++) {
            for ($j = 0; $j < 4; $j++) {
                ...
                Biodata::create([
                    'user_id' => User::count(),
                    'name' => $encryptor->factory_encrypt($encryption_method[$i], $encryption_mode[$j], '1N1password', 'Name'),
                    'gender' => $encryptor->factory_encrypt($encryption_method[$i], $encryption_mode[$j], '1N1password', 'Male'),
                    'nationality' => $encryptor->factory_encrypt($encryption_method[$i], $encryption_mode[$j], '1N1password', 'Indonesia'),
                    'religion' => $encryptor->factory_encrypt($encryption_method[$i], $encryption_mode[$j], '1N1password', 'Islam'),
                    'marital_status' => $encryptor->factory_encrypt($encryption_method[$i], $encryption_mode[$j], '1N1password', 'Single'),
                ]);
                ...
                File::create([
                    'user_id' => User::count(),
                    'filename' => $encryptor->factory_encrypt($encryption_method[$i], $encryption_mode[$j], '1N1password', $filename),
                    'filecode' => $filename,
                    'filetype' => 'document',
                ...
                fwrite($file_dest, $encryptor->factory_encrypt($encryption_method[$i], $encryption_mode[$j], '1N1password', $raw));
                fclose($file_dest);
